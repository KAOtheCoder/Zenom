...
void RadialGaugeSettingWidget::initGauge( AbstractGauge* pGauge )
{
    ...
    if ( radialGauge )
    {
        ...
        // Scale
        radialGauge->setScale( ui->rangeMin->value(), ui->rangeMax->value() );
        radialGauge->setOrigin( ui->startAngle->value() );
        radialGauge->setScaleArc( 0, ui->sweepAngle->value() );

        // Ticks
        radialGauge->setScaleMaxMajor(ui->majorTick->value());
        radialGauge->setScaleMaxMinor(ui->minorTick->value());
