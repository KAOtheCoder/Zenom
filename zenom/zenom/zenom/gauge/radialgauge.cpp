    ...
    : QwtDial(pParent)
{
    ...
    setScaleMaxMajor(12);
    setScaleMaxMinor(4);
    setNeedle( SIMPLE_NEEDLE, NEEDLE_STYLE_1, Qt::red, QColor( Qt::gray ).lighter( 130 ) );
    setTextColor( Qt::white );
    setBackgroundColor( QColor( Qt::darkGray ).darker( 150 ) );
    setLineWidth( 4 );
...
{
    // Scale
    setScale( pSettings.value("minValue", 0).toDouble(), pSettings.value("maxValue", 240).toDouble() );
    setOrigin( pSettings.value("origin", 135).toDouble() );
    setScaleArc( 0, pSettings.value("sweepAngle", 270).toDouble() );

    // Ticks
    setScaleMaxMajor(pSettings.value("scaleMaxMajor", 12).toDouble());
    setScaleMaxMinor(pSettings.value("scaleMaxMinor", 4).toDouble());
...
void RadialGauge::setBackgroundColor( const QColor& pColor )
{
    ...
    p.setColor( QPalette::Window, pColor.darker( 150 ) );
    p.setColor( QPalette::Mid, pColor.darker( 110 ) );
    p.setColor( QPalette::Light, pColor.lighter( 170 ) );
    p.setColor( QPalette::Dark, pColor.darker( 170 ) );
